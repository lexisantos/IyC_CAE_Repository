...
def data_cut(T, I, series, t_irr, Ts: float=1, version: str='old', abs_path: str=os.getcwd(), signal: str='patron', emisorpat: str='Rh'):
    ...
    Ts : float, seg
        Periodo de sampleo
    version: str, default to 'old'
        Sólo se usa para spnds calibrados. Dar la versión del software usado para levantar los datos del patrón.
    abs_path:
        Ubicación absoluta de las carpetas con los datos.
    signal : str, 
...
def data_cut_save(lista_cat, lista_emisores, listaSPND, lista_versiones, lista_Ts, lista_tirr, path_abs: str=os.getcwd()):
    ...
    for emisor in emisores:
        for subpath in ['', '_I0', 'Diff']:
            ...
            if not os.path.exists(newpath):
                os.makedirs(newpath)
    SPND_new = listaSPND[np.where(lista_versiones == 'new')]
    listaSPND = np.delete(listaSPND, np.where(lista_versiones == 'new'))
    ...
    for jj, spndno in enumerate(listaSPND):
        ...
        datoscortados, diff =  data_cut(T, I, spndno, lista_tirr[jj], lista_Ts[jj], 
                                        path_abs, lista_cat[jj], lista_emisores[jj])
    ...
    for jj, spndno in enumerate(SPND_new):
        T, I, I_patron = load_data(spndno, lista_emisores[jj], path_abs, version='new')
        det_cut, diff = data_cut(T, I, spndno, lista_tirr[jj], lista_Ts[jj], 
                                        path_abs, lista_cat[jj], lista_emisores[jj],
                                        version = 'new')
        pat_cut, diff = data_cut(T, I_patron, spndno, lista_tirr[jj], lista_Ts[jj], 
                                        path_abs, lista_cat[jj], lista_emisores[jj],
                                        version = 'new')
    
# data_cut_save. Revisar qué hacer cuando es una versión nueva.  
# SPND_new = SPNDno[np.where(lista_versiones == 'new')]
# SPNDno = np.delete(SPNDno, np.where(lista_versiones == 'new'))
# Que el default de categoria sea patron, porque es el que más quiero cortar al principio,
# y veo si hay errores. Incluso para estudiar DIM
##
